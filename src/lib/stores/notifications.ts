...
export const addNotification = (message: string, options: Partial<Omit<Notification, 'id' | 'message'>>) => {
  ...
  if (notification.timeout) setTimeout(() => dismissNotification(notification.id), notification.timeout);
};
