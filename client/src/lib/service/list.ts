...
export const sync = async (list: ShoppingList) => {
    ...
    try {
        ...
        const parsedResponseData = syncResponseSchema.safeParse(await response.json());
        if (!parsedResponseData.success) {
            // Malformed server response data

            addNotification("Malformed response data when synching, reach out to one of the app's maintainers", {
                type: "error",
                dismissible: true,
                timeout: 5000,
            });
        ...
        const responseList: ShoppingList = ShoppingList.fromJSON(parsedResponseData.data.list);
        const responseListId = responseList.id;

        const locallyStoredListData = await localforage.getItem<ShoppingListJSON>(responseListId);

        console.log("locallyStoredListData", locallyStoredListData);

        if (locallyStoredListData !== null) {
            const locallyStoredList = ShoppingList.fromJSON(locallyStoredListData);

            console.log("local", locallyStoredList, "remote", responseList);

            locallyStoredList.merge(responseList);
            
            console.log("Merged remote list into local list", locallyStoredListData);
